...
pub async fn latest_price_delta() -> f64 {
    ...
    loop {
        let price1 = price(sol_pubkey, usdc_pubkey, 1.0).await.unwrap().data.price;
        let price2 = price(sol_pubkey, usdc_pubkey, 1.0).await.unwrap().data.price;

        if price1 != price2 {
            println!("{}", (price1 - price2).abs());
